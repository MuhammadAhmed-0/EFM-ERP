...
const ClientChallanInvoicePrint = () => {
  const { id } = useParams();
    ...
    const fetchChallan = async () => {
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/api/payments/client/challans/${id}`,
          {
            headers: {
  ...
  return (
    <div className="challan-print-wrapper">
      <div className="challan-print-container">
        ...
        {/* {challanData.paymentHistory &&
          Array.isArray(challanData.paymentHistory) &&
          challanData.paymentHistory.length > 0 && (
            <div style={{ marginTop: "20px" }}>
              <h3
                style={{
                  ...
                  marginBottom: "15px",
                  fontSize: "1.2em",
                  borderBottom: "2px solid #1f3d61",
                  paddingBottom: "5px",
                }}
              >
                Payment History ({challanData.paymentHistory.length} payments)
              </h3>
              <table className="challan-fee-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {challanData.paymentHistory.map((payment, index) => (
                    <tr key={payment._id || index}>
                      ...
                      <td className="challan-fee-cell">
                        {payment.receivedByName || "Unknown"}
                      </td>
                      <td className="challan-fee-cell">
        ...
        <div className="challan-footer">
          <div>
            ...
            <strong
              className={`status-tag ${challanData.status
                .toLowerCase()
                .replace("_", "-")}`}
              style={{
                color: getStatusColor(challanData.status),
              }}
            >
...
export default ClientChallanInvoicePrint;
