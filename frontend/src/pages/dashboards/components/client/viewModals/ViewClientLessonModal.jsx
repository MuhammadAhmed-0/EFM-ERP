...
const ViewClientLessonModal = ({ schedule, open, onClose }) => {
  ...
  const fetchStudentLessons = async () => {
    try {
      ...
      const studentId =
        schedule.studentNames?.[0]?._id || schedule.students?.[0]?._id;
      const teacherId = schedule.teacherId;
      const subjectId = schedule.subject;

      if (!studentId || !teacherId || !subjectId) {
        showNotification(
          "Missing required information to fetch lessons",
          "error"
        );
        return;
      }
      const response = await axios.get(
        `${BASE_URL}/api/schedules/client/student-lessons/${studentId}/${teacherId}/${subjectId}`,
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        showNotification(
          response.data.message || "Failed to fetch lessons",
          "error"
        );
      }
    } catch (error) {
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>
                    Added by {lesson.teacherName} on{" "}
                    {format(new Date(lesson.addedAt), "dd/MM/yyyy HH:mm")}
                  </div>
                </Box>
              </Box>
            ))}
...
export default ViewClientLessonModal;
